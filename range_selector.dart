...
class _SfRangeSelectorState extends State<SfRangeSelector>
  ...
  AnimationController tooltipAnimationStartController;
  AnimationController tooltipAnimationEndController;
...
class _RenderRangeSelector extends RenderBaseSlider
  ...
  Animation<double> _tooltipStartAnimation;

  Animation<double> _tooltipEndAnimation;
  ...
  void _reverseTooltipAnimation() {
    ...
    if (isInteractionEnd &&
      ...
      if (_state.tooltipAnimationEndController.status ==
          AnimationStatus.dismissed) {
        willDrawTooltip = false;
      }
    }
  ...
  void _drawOverlayAndThumb(
    ...
    overlayShape.paint(
      ...
      themeData: sliderThemeData,
      currentValues: _values,
      animation: _overlayStartAnimation,
    );
    overlayShape.paint(
      context,
      endThumbCenter,
      ...
      themeData: sliderThemeData,
      currentValues: _values,
      animation: _overlayEndAnimation,
    );
  ...
  void _drawTooltip(
    ...
    if (willDrawTooltip) {
      ...
      dynamic actualText =
          getValueFromPosition(startThumbCenter.dx - offset.dx);
      ...
      tooltipShape.paint(context, startThumbCenter,
          Offset(actualTrackOffset.dx, tooltipStartY), textPainter,
          parentBox: this,
          sliderThemeData: sliderThemeData,
          paint: paint,
          animation: _tooltipStartAnimation,
          trackRect: trackRect);

      actualText = getValueFromPosition(endThumbCenter.dx - offset.dx);
      ...
      tooltipShape.paint(context, endThumbCenter,
          Offset(actualTrackOffset.dx, tooltipStartY), textPainter,
          parentBox: this,
          sliderThemeData: sliderThemeData,
          paint: paint,
          animation: _tooltipEndAnimation,
          trackRect: trackRect);
    }
  }
  ...
  void _handleHover(PointerHoverEvent details) {
    ...
    final double endPosition =
        getFactorFromValue(actualValues.end) * actualTrackRect.width +
            actualTrackRect.left;
    final double leftThumbHover = (cursorPosition - startPosition).abs();
    final double rightThumbHover = (cursorPosition - endPosition).abs();
    if (dragMode == SliderDragMode.betweenThumbs) {
    ...
    } else if (dragMode == SliderDragMode.both) {
      if (cursorPosition >= startPosition && cursorPosition <= endPosition) {
        _state.overlayStartController.forward();
        _state.overlayEndController.forward();
        if (enableTooltip) {
      ...
      if (rightThumbHover > leftThumbHover) {
        _state.overlayStartController.forward();
        _state.overlayEndController.reverse();
        if (enableTooltip) {
          willDrawTooltip = true;
